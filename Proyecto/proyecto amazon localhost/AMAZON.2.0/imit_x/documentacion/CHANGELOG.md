...
## [2.0.0] - 2025-ENERO (ÚLTIMA ACTUALIZACIÓN)

### ✨ INTEGRACIÓN COMPLETA DE FUNCIONES DE PORFIRIO

#### **Fase 1: Autenticación & AuthContext (✅ COMPLETADO)**
- ✅ Creación de `src/services/auth.service.ts`
  - Singleton service para autenticación
  - Métodos: login, signup, logout, getCurrentSession, isAuthenticated, switchAccount
  - Almacenamiento de sesión en localStorage con clave `auth_data`
  - Soporte para roles: 'client' | 'seller'

- ✅ Creación de `src/context/AuthContext.tsx`
  - Provider de React Context para estado de autenticación
  - Hook exportado: `useAuth()`
  - State: user, role, loading, isAuthenticated
  - Carga automática de sesión al montar el componente

- ✅ Actualización de `src/App.tsx`
  - Envolvimiento de rutas con `<AuthProvider>`
  - Importación de ConfiguracionCuenta page
  - Ruta `/configuracion` agregada

- ✅ Actualización de `src/pages/Login.tsx`
  - Reemplazo de fetch manual con `useAuth().login()`
  - Integración con AuthContext

- ✅ Actualización de `src/pages/Register.tsx`
  - Reemplazo de fetch manual con `useAuth().signup()`
  - Integración con AuthContext

#### **Fase 2: Componentes Mejorados (✅ COMPLETADO)**
- ✅ Actualización de `src/components/AmazonHeader.tsx`
  - Integración con `useAuth()` hook
  - Displays de usuario actual y rol con badge

- ✅ Creación de `src/pages/ConfiguracionCuenta.tsx` (NUEVA)
  - Avatar del usuario con opción cambiar foto
  - Formulario con: nombre, email, teléfono, dirección
  - Toggle para notificaciones
  - Selector de tema: claro/oscuro/automático
  - Botón "Cambiar de Cuenta" con alert de confirmación
  - Botón "Cerrar Sesión" con alert de confirmación
  - Integración completa con AuthContext

#### **Fase 3: Testing & Compilación (✅ EXITOSO)**
